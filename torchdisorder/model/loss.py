# # ---------------------------------------------------------------------------
# # model/loss.py – Augmented‑Lagrangian loss builder
# # ---------------------------------------------------------------------------
# # Implements chi² metrics plus an Augmented Lagrangian scheme that
# # automatically updates the weighting between correlation and scattering
...
# import torch
# # from gbsd.spectra import (
# #     get_neutron_total_rdf,
# #     get_neutron_total_correlation_function,
# #     get_neutron_total_scattering_sf,
# # )

# from model.rdf import  SpectrumCalculator
# __all__ = ["AugLagLoss"]
...
#     If *uncertainty* is a scalar, broadcast to estimate.shape.
#     """
#     if isinstance(uncertainty, float) or isinstance(uncertainty, int):
#         uncertainty = torch.tensor(uncertainty, device=estimate.device)
#     return torch.sum((estimate - target) ** 2 / (uncertainty**2))


# @dataclass
# class AugLagHyper:
#     """Hyper‑parameters controlling the Augmented Lagrangian."""

#     rho: float = 1e-3         # initial penalty parameter
#     rho_factor: float = 5.0   # multiply rho by this if constraint stagnates
#     tol: float = 1e-4         # |g(x)| below which we consider the constraint satisfied
#     update_every: int = 10    # iterations between ρ updates
#     scale_scatt: float = 0.02 # front‑factor for scattering χ² (keeps magnitudes similar)


# class AugLagLoss:
#     """Callable loss object that tracks λ and ρ and updates them online.

#     Each call returns the current Augmented Lagrangian value and *also* updates
#     the Lagrange multiplier (λ) following

#         λ ← λ + ρ · g(x)

#     where g(x) is the correlation χ² (treated as an equality‑to‑zero constraint).
#     After *update_every* steps, if |g(x)| has not dropped below *tol*, we increase ρ.
#     """
...
#     ) -> None:
#         self.chemical_symbols = chemical_symbols  # ordering must match positions tensor
#         self.r_bins = r_bins
#         self.q_bins = q_bins
#         self.T_r_target = T_r_target
#         self.F_q_target = F_q_target
#         self.F_q_uncert = F_q_uncert
#         self.device = torch.device(device)

#         # Lagrange multiplier λ (scalar) and penalty ρ
#         self.lambda_corr = torch.zeros(1, device=self.device)
#         self.rho = torch.tensor(hyper.rho, device=self.device)
#         self.hyper = hyper

#         # Bookkeeping
#         self.iter_counter = 0
#         self._g_prev: torch.Tensor | None = None
#         self.time_spent = 0.0
...
#         Parameters
#         ----------
#         positions : (N,3) torch.Tensor, **requires_grad=True**
#         cell      : (3,3) torch.Tensor with lattice vectors
#         """
#         start = time.perf_counter()

#         # sc = SpectrumCalculator( neutron_scattering_lengths=scattering_data["neutron_scattering_lengths"],xray_form_factor_params=scattering_data["xray_form_factor_params"],)

#         # rdf = sc.compute_neutron_rdf(...)
#         # correlation = sc.compute_neutron_correlation(...)
#         # sf = sc.compute_neutron_sf(...)
        
#         # 1) forward RDF / correlation / scattering
#         total_rdf = get_neutron_total_rdf(
#             self.chemical_symbols, positions, cell, self.r_bins
#         )
#         g_corr = get_neutron_total_correlation_function(
#             total_rdf, self.r_bins, self.chemical_symbols, cell
#         )
#         S_Q = get_neutron_total_scattering_sf(
#             total_rdf, self.r_bins, self.q_bins, self.chemical_symbols, cell
#         )

#         # 2) elemental χ² pieces
#         corr_chi2 = chi_squared(g_corr, self.T_r_target, uncertainty=0.05) / len(g_corr)
#         scatt_chi2 = (
#             chi_squared(S_Q, self.F_q_target, self.F_q_uncert) / len(self.F_q_target)
#         )

#         # 3) Augmented Lagrangian L = w_s * f_s + λ g + ½ρ g², where we treat g ≡ corr_chi2
#         g_val = corr_chi2.detach()  # constraint value (no grad in λ update)
#         lagrangian = (
#             self.hyper.scale_scatt * scatt_chi2
#             + self.lambda_corr * corr_chi2
#             + 0.5 * self.rho * corr_chi2**2
#         )

#         # 4) Update λ each iteration (outside autograd)
#         with torch.no_grad():
#             self.lambda_corr += self.rho * g_val

#         # 5) Optionally adjust ρ every *update_every* iters
#         self.iter_counter += 1
...
#         return {
#             "lambda_corr": self.lambda_corr.item(),
#             "rho": self.rho.item(),
#             "iter": self.iter_counter,
#             "time_spent_s": self.time_spent,
#         }


# model/loss.py – Augmented‑Lagrangian loss builder with tetrahedral order parameter
# -----------------------------------------------------------------------------
# Combines scattering χ², correlation χ² (as constraint), *and* a tetrahedral
# order‑parameter term q into a differentiable loss. λ and ρ are updated online
# to enforce the correlation constraint, while scale_scatt and scale_q are updated
# like augmented Lagrange multipliers.
...
# ) -> Tensor:
#     """
#     Differentiable average tetrahedral order parameter ⟨q⟩.
#
#     Returns a 0‑D tensor on the same device so autograd can propagate
#     gradients back to *positions* and *cell*.
#     """
#     # positions: Tensor,  # (N, 3) requires_grad = True
#     # symbols: list[str],  # chemical symbols in the same order
#     # cell: Tensor,  # (3,)  or  (3,3)
...
#
#     # --- ensure 3×3 cell matrix ----------------------------------------
#     if cell.ndim == 1:                 # (a, b, c)  ->  diag(a,b,c)
#         cell = torch.diag(cell)
#     elif cell.ndim == 2 and cell.shape == (3, 1):   # 3×1 column
#         cell = torch.diagflat(cell.flatten())
#     # else: assume already (3,3)
#
#     # --- neighbour list -------------------------------------------------
...
#     )
#     i, j = mapping
#     rij = positions[j] + shifts @ cell - positions[i]
#
#     # --- masks for central & neighbour atoms ---------------------------
#     central_idx   = torch.tensor(
#         [k for k, s in enumerate(symbols) if s == central],
#         dtype=torch.long,
#         device=device,
#     )
#     neigh_mask_all = torch.tensor(
...
#
#     q_vals = []
#     for ic in central_idx:
#         mask  = (i == ic) & neigh_mask_all[j]
#         vecs  = rij[mask]
#         n     = vecs.shape[0]
#         if n < 3:
#             q_vals.append(torch.zeros((), device=device, dtype=dtype))
#             continue
#
#         v_norm = vecs / vecs.norm(dim=1, keepdim=True)
#         cosmat = v_norm @ v_norm.T                       # pairwise cosines
#         cos_ij = cosmat[torch.triu_indices(n, n, 1)]
#         q_i    = (cos_ij + 1.0 / 3.0).square().sum()
#
#         q_vals.append(1.0 - 6.0 * q_i / (n * (n - 1)))
#
#     return torch.stack(q_vals).mean()          # tensor, keeps gradient

#
# def mean_tetrahedral_q_from_tensors(
#     positions: torch.Tensor,          # (N,3)  requires_grad=True
#     symbols: list[str],               # python list length N
#     cell: torch.Tensor,               # whatever → normalised to (3,3)
#     *,
#     central: str = "Si",
#     neighbour: str = "O",
#     cutoff: float = 3.5,
...
# -----------------------------------------------------------------------------
# Augmented‑Lagrangian loss class with dynamic scale updates
# -----------------------------------------------------------------------------
#
# class AugLagLoss:
#     def __init__(
...
#     ) -> None:
#         self.spec_calc = spectrum_calc
#         self.rdf_data = rdf_data
#         self.hyper = hyper
#         self.device = torch.device(device)
#
#         self.lambda_corr = torch.zeros(1, device=self.device)
#         self.rho = torch.tensor(float(hyper.rho), device=self.device)
#         self.scale_scatt = torch.tensor(float(hyper.scale_scatt_init), device=self.device)
#         self.scale_q = torch.tensor(float(hyper.scale_q_init), device=self.device)
#
#         self.iter_counter = 0
#         self.time_spent = 0.0
#
#     def __call__(self, positions: torch.Tensor, cell: torch.Tensor, symbols: List[str]) -> torch.Tensor:
#         start = time.perf_counter()
#
#         G_r = self.spec_calc.compute_neutron_rdf(symbols, positions, cell, self.rdf_data.r_bins)
#         T_r = self.spec_calc.compute_neutron_correlation(G_r, self.rdf_data.r_bins, symbols, cell)
#         S_Q = self.spec_calc.compute_neutron_sf(G_r, self.rdf_data.r_bins, self.rdf_data.q_bins, symbols, cell)
#
#         #  elemental χ² pieces
#         chi2_corr = chi_squared(T_r, self.rdf_data.T_r_target, 0.05) / T_r.numel()
#         chi2_scatt = chi_squared(S_Q, self.rdf_data.F_q_target, self.rdf_data.F_q_uncert) / S_Q.numel()
#
#         q_loss = torch.tensor(0.0, device=self.device)
#         if self.scale_q > 0:
#             q_val = mean_tetrahedral_q(positions, symbols, cell)
#             q_loss = chi_squared(q_val, torch.tensor(self.hyper.q_target, device=self.device), self.hyper.q_uncert)
#
#         #  Augmented Lagrangian L = w_s * f_s + λ g + ½ρ g², where we treat g ≡ corr_chi2
#         g_val = chi2_corr.detach()
#         L = (
...
#             + self.scale_q * q_loss
#         )
#         #  Update λ each iteration (outside autograd)
#         with torch.no_grad():
#             self.lambda_corr += self.rho * g_val
#
#         #  Optionally adjust ρ and scales every *update_every* iters
#
#         self.iter_counter += 1
...
#             "lambda_corr": self.lambda_corr.item(),
#             "rho": self.rho.item(),
#             "scale_scatt": self.scale_scatt.item(),
#             "scale_q": self.scale_q.item(),
#             "iter": self.iter_counter,
#             "time_spent_s": self.time_spent,
#         }
#
...
#
#     def forward(self, desc: Dict[str, torch.Tensor]) -> torch.Tensor:
#
#         chi2_corr = chi_squared(desc["T_r"], self.rdf.T_r_target, 0.05) / desc["T_r"].numel()
#         chi2_scatt = chi_squared(desc["S_Q"], self.rdf.F_q_target, self.rdf.F_q_uncert) / desc["S_Q"].numel()
#         q_loss = chi_squared(
...
#         self.last_stats = {
#             "loss": total.detach(),
#             "chi2_corr": chi2_corr.detach(),
#             "chi2_scatt": chi2_scatt.detach(),
#             "q_loss": q_loss.detach(),
#             "scale_q": self.scale_q.detach(),
#             "scale_scatt": self.scale_scatt.detach(),
#             "rho": self.rho.detach(),
#             "lambda_corr": self.lambda_corr.detach(),
#         }
#
#         return total
#
#     def update_penalties(self) -> None:
#         g_val = self.last_stats["chi2_corr"]
#         with torch.no_grad():
#             self.lambda_corr += self.rho * g_val
#             self.iter_counter += 1
#             if self.iter_counter % self.hyper.update_every == 0 and g_val.abs() > self.hyper.tol:
