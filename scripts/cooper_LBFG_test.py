...
def main(cfg: DictConfig) -> None:
    ...
    # CREATE OPTIMIZERS
    primal_optimizer = torch.optim.LBFGS(  # LBFGS
        primal_params,
        lr=1.0,
        max_iter=5,
        history_size=10,
    ...
    dual_optimizer = torch.optim.SGD(
        ...
        lr=1.0,  # ← Change to match penalty_rho (was 1e-2)
        maximize=True
    )

    # CREATE COOPER OPTIMIZER
    cooper_opt = AlternatingPrimalDualOptimizer(  # ← Change from SimultaneousOptimizer
        cmp=cooper_problem,
    ...
    for step in range(cfg.max_steps):
        # AlternatingPrimalDualOptimizer does alternating updates in one call
