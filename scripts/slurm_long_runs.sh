#!/bin/bash
# =============================================================================
# TorchDisorder — long production runs for SiO2 and GeO2
# =============================================================================
#
# Both systems now start from a published, independently validated glass, with
# constraints regenerated against that exact structure and F_IS constrained
# alongside cn and tet.
#
...
#SBATCH --output=logs/long_%A_%a.out
#SBATCH --error=logs/long_%A_%a.err
...
SYSTEMS=(
  "SiO2|data=SiO2 structure=silica"
  "GeO2|data=GeO2 structure=GeO2"
)
...
FINAL=$(ls -d outputs/${LABEL}_long_*/*/final_results/final_structure.cif 2>/dev/null | tail -1)
if [ -n "$FINAL" ]; then
  echo; echo "===== independent scoring ====="
  $CONDA_PYTHON scripts/compare_to_literature.py --test "$FINAL" --reference "$REF" \
      --system "$LABEL" --label "${LABEL}_long" 2>/dev/null || true
  $CONDA_PYTHON scripts/compare_order_params.py --reference "$REF" --test "$FINAL" \
      --labels "${LABEL}_long" --central "$C" --neighbour "$N" --cutoff "$CUT" 2>/dev/null || true
fi
